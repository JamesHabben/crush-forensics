...
- **OGG / Opus playback — cross-platform via PyAV** — OGG Vorbis and Opus files are now decoded to raw PCM using PyAV (bundled FFmpeg) and played through Qt's `QAudioSink`, bypassing platform codec limitations on macOS (AVFoundation) and Windows (Media Foundation) which do not natively support Opus. WhatsApp (`.opus`) and Telegram (`.ogg`) voice notes play on all platforms. Seek and time display work on the PCM path; video and all other audio formats continue through `QMediaPlayer` unchanged.
