...
- **Realm viewer: Link/LinkList relationships are now resolvable, in the UI and in SQL** — the Schema tab shows each Link/LinkList column's target table (e.g. `attachments: linklist → class_AttachmentLocalDto`); a new **Views** tab lets you pick a table, choose per Link/LinkList column which columns of the linked table to pull in, and open the fully resolved table as a new tab with no SQL required; every table with a Link/LinkList column also gets a matching `v_<table>` SQL view doing the same resolution (`SELECT * FROM v_class_MessageAttributesLocalDto`) for scripted queries, alongside `json_each()` support for exact per-element matching. Table-grid cells holding a List/Set/LinkList value are now colour-flagged with an explanatory tooltip instead of looking like plain text.
- **Realm viewer: Views tab results now open with a working SQL box** — opening a resolved view (the "Open View" button) now backs that one tab with a real temp SQLite file, same as the Tables tab, instead of leaving it a static, unqueryable grid. The tab's own SQL box can run `SELECT col_a, col_c FROM ...` to pick, reorder, or filter the resolved columns for display or CSV export — scoped to that single opened tab only, without touching the Views tab's own configuration or any other tab. Dotted column names from an expanded to-one Link (e.g. `messageAttributes.subject`) work as quoted SQL identifiers.
- **Realm viewer: Views tab now resolves multi-hop Link/LinkList chains** — a chosen target column that's itself a Link/LinkList (e.g. `message.attachments → attachment`, where `attachment.uploader` links on to `participant`) now expands into its own checklist right in the tree instead of only ever resolving the first hop, so a chain across several tables opens as one resolved view instead of leaving the second hop as a raw ObjKey. Populated lazily (only once a node is actually expanded) and capped at 8 hops, with a table already on the current chain never offered again, so a densely cross-referenced or cyclic schema can't hang or blow up the tree. A resolved to-one Link now lands in its own named column per selected field (e.g. `messageAttributes.subject`, `messageAttributes.spamInfo.category` for a further-nested link) instead of every selected field being crammed into one flattened text cell — found to be unreadable on a real 25-column `MessageAttributesLocalDto` table. A to-many LinkList still collapses to one combined text cell, since a variable-length list of targets per row can't become a fixed set of columns without multiplying rows. The `v_<table>` SQL views stay single-hop by design — chaining those further is already straightforward by hand with `json_each()` once you're writing SQL anyway.
