...
- **BLOB Inspector — multi-step decode pipeline** — the inspector now shows three columns side-by-side: a *Decode pipeline* on the left (add unlimited byte→byte transform steps: Base64, Base64url, Hex→Bytes, zlib, gzip, lzfse), an *Interpretations* list in the middle with a three-tier confidence system (✓ confident / ~ permissive / gray failed), and a *Content view* on the right that updates with a single click. The auto-selection logic restores the previous format only when it still produces output after a pipeline change. New dependency: `lzfse>=0.4.2`.
- **Paste & Decode — redesigned as BLOB Inspector entry point** — the *Tools → Paste & Decode…* dialog now embeds the full BLOB Inspector panel directly below the input field. Paste hex, base64, or text; the panel updates live as you type. All decode steps and interpretations added to the BLOB Inspector are automatically available here too.
