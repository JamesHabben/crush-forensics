...
- **Realm dual top-ref decoding** — the parser now decodes schema and table data from *both* top-level references (the active/current snapshot and the inactive/previous snapshot), not just the active one. This mirrors how the SQLite WAL viewer exposes superseded page versions. In the Tables tab, inactive-ref tables appear alongside active ones with a `[prev ref N]` prefix; the underlying temporary SQLite database stores them as `_prev_<tablename>` so cross-snapshot SQL comparisons are possible (e.g. `SELECT … FROM class_Evidence e JOIN _prev_class_Evidence p ON e._objkey = p._objkey WHERE e.col != p.col`). The Summary row now includes a **Ref** column identifying whether each table comes from the active or the previous snapshot.
