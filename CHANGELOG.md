...
- **Format knowledge base — AVIF added; HEIC/HEIF and JPEG XL corrected** — AVIF (AV1 Image File Format) is now included in the format knowledge base with forensic background, ISOBMFF `avif`/`avis` brand magic bytes, platform list, and reference links. HEIC/HEIF and JPEG XL entries were previously missing a `parser_class` reference, causing the "Open in viewer" action to fall through to the hex viewer rather than the image viewer; both are now correctly linked to `ImageParser`.
- **OGG / Opus / AMR playback — cross-platform via PyAV** — OGG Vorbis, Opus, and AMR files are now decoded to raw PCM using PyAV (bundled FFmpeg) and played through Qt's `QAudioSink`, bypassing platform codec limitations on macOS (AVFoundation) and Windows (Media Foundation) which do not natively support these codecs. WhatsApp (`.opus`) and Telegram (`.ogg`) voice notes play on all platforms. Seek and time display work on the PCM path; video and all other audio formats continue through `QMediaPlayer` unchanged.
- **OGG / Opus / AMR metadata in Properties panel** — the Properties panel now shows codec, sample rate, channels, duration, and Vorbis comment tags (encoder, title, artist, date, comment) for OGG, Opus, and AMR files. The `Encoder` field is forensically relevant: it identifies the originating app (e.g. `libopus` version from WhatsApp or Signal).
- **Media parser — magic-byte fallback for renamed audio** — OGG and AMR files with non-standard extensions (e.g. voice notes saved as `.bin`) are now opened in the media viewer based on content (`OggS` / `#!AMR` magic bytes) rather than extension alone.
- **OGG / Opus type detection** — OGG Vorbis and Opus audio files now receive specific type labels (`OGG`, `Opus`) detected from the Ogg page codec identification bytes (`\x01vorbis` / `OpusHead` at page payload offset 28), rather than the generic `Media` label. `type:opus` in the filesystem panel filter now matches WhatsApp and Telegram voice notes specifically; `type:media` continues to match all audio and video. The format info panel correctly distinguishes Opus from generic OGG containers.
- **Format info panel — most-specific magic match** — `identify()` now returns the format with the longest matching magic pattern rather than the first match in DB order; this prevents generic container signatures (e.g. `OggS`) from shadowing more specific codec identifiers (e.g. `OpusHead`) and is robust against future format additions.
